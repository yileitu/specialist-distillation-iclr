#!/usr/bin/env python3
"""
Generate a LLaMA-Factory LoRA training YAML configuration

Usage:
    python generate_lora_yaml.py \
        --model_path /path/to/model \
        --dataset my_dataset \
        --output_dir /path/to/output \
        --yaml_output_path /path/to/training.yaml

This script generates configuration for LoRA fine-tuning.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Generate a LLaMA-Factory LoRA training configuration')
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-4,
        help='Learning rate (LoRA commonly uses a higher rate)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default='all',
        help='LoRA target modules (default: all linear layers)'
    )
    parser.add_argument(
        '--lora_dropout',
        type=float,
...
def generate_lora_config(args) -> Dict[str, Any]:
...
def main():
    ...
    config = generate_lora_config(args)
    ...
    print(f"  Fine-tuning type: lora")
    print(f"  LoRA rank: {args.lora_rank}")
    print(f"  LoRA Alpha: {lora_alpha}")
    print(f"  LoRA target: {args.lora_target}")
    print(f"  LoRA Dropout: {args.lora_dropout}")
    print(f"  RSLoRA: {args.use_rslora}")
